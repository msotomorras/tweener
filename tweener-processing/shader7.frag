...
if(x<.99){
result=getY(x);
//result+=sin(result*100)*.0052;

////result *= sin(integrate*.1);
}
...
{   float thickness = .5;
    ...
    //thickness = .5 +thickness*.3;
    float d = sdSphere( pos,thickness );
...
//      brdf += 2.20*amb*vec3(0.20,0.22,0.26)*ao;
        ...
        brdf += 01.20*bac*vec3(0.15,0.215,0.515)*ao;
        ...
        col = col*brdf + spe + 0.2*fre*(0.5+0.5*col);
...
{   vec2 q = gl_FragCoord.xy/iResolution.xy;
    ...
    ro = vec3(cameraX,cameraY,6+move);
